...
Uses LM Studio local LLM with tool calling capabilities for natural language processing.
"""
...
class LLMTransactionChatBot:
    """LLM-powered natural language interface for transaction search"""
    ...
    def call_llm(self, user_message: str) -> Dict[str, Any]:
        """Call LM Studio LLM with tool support"""
...
def main():
    st.title("🤖 Banking Transaction Chat Assistant (LLM-Powered)")
    st.markdown("Ask me about your banking transactions - powered by your local LLM!")
    ...
    with st.sidebar:
        ...
        if st.button("Update LM Studio URL"):
            st.session_state.llm_chatbot.lm_studio_url = lm_studio_url
            st.success("LM Studio URL updated!")
        ...
        if st.button("Test LLM Connection"):
            try:
                ...
                if response.status_code == 200:
                    ...
                    st.error("❌ LM Studio responded with error")
            except Exception as e:
            ...
            for example in examples:
                if st.button(f"💬 {example}", key=f"llm_example_{hash(example)}", use_container_width=True):
                    ...
                    with st.chat_message("assistant"):
                        with st.spinner("🤖 Processing with LLM... Understanding your query and searching transactions..."):
    ...
    if (len(st.session_state.llm_messages) > 0 and 
        ...
        # Process the query with LLM and show spinner
        with st.chat_message("assistant"):
            with st.spinner("🤖 Processing with LLM... Understanding your query and searching transactions..."):
                chatbot = st.session_state.llm_chatbot

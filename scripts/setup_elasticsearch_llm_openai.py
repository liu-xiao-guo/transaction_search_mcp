...
for banking transactions using OpenAI for realistic data generation.
"""
...
OPENAI_MODEL = os.getenv("OPENAI_MODEL", "gpt-4-turbo-preview")  # Default to gpt-4-turbo-preview for better quality
...
class OpenAIClient:
    ...
    def generate_transaction_data(self, category: str, amount: float, location: str, date: str) -> Dict[str, Any]:
        """Generate realistic transaction data using OpenAI"""
...
def generate_test_data_with_llm(num_transactions: int = 500) -> List[Dict[str, Any]]:
    """Generate realistic test banking transaction data using OpenAI"""
    ...
    if use_llm:
        print(f"✅ Connected to OpenAI - using AI for realistic transaction generation with {OPENAI_MODEL}")
    else:
        print("⚠️  OpenAI API not available - using fallback data generation")
        print("   Make sure OPEN_AI_KEY is set in your .env file")
